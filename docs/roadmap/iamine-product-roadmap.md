...
`AGENT-TIMEOUT-CANCEL-ENFORCEMENT-001` is active at
`APPROVED FOR MERGE` after Architecture, implementation, local validation,
exact-tree field QA, and final Architecture review. The remaining eight rows
stay `PROPOSED`; no later row is authorized in bulk.
