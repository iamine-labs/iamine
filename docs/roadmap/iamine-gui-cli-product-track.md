...
| DASHBOARD-NODES-MOCK-001 | ARCHITECTURE REVIEW REQUIRED | Additive typed, deterministic Nodes preview with complete Mac frontend QA; no discovery, node connectivity, machine identifiers, runtime policy, or operational actions. Core diff is empty; this row does not alter the real-integration sequence. |
