pub(crate) fn usage_text() -> &'static str {
    "Uso:\n  iamine-node --worker [--port=N] [--cpu=N] [--ram=N] [--gpu] [--bootnode ADDR] [--wan-peer ADDR] [--relay-policy MODE] [--relay-peer ADDR] [--testnet-admission MODE] [--testnet-allow-peer PEER_ID]\n  iamine-node worker lifecycle [install|start|stop|restart|readiness|recover|status] [--port=N] [--json]\n  iamine-node node config [status|migrate|rollback] [--path PATH] [--yes] [--json]\n  iamine-node node identity [status|init] [--path PATH] [--json]\n  iamine-node support bundle [--output PATH] [--json]\n  iamine-node agents node-doctor --package-root PATH [--json]\n  iamine-node --relay [--bootnode ADDR] [--wan-peer ADDR] [--relay-policy MODE] [--relay-peer ADDR] [--testnet-admission MODE] [--testnet-allow-peer PEER_ID]\n  iamine-node --broadcast <type> <data> [--required-model MODEL] [--bootnode ADDR] [--wan-peer ADDR] [--relay-policy MODE] [--relay-peer ADDR] [--testnet-admission MODE] [--testnet-allow-peer PEER_ID]\n  iamine-node models catalog\n  iamine-node models list\n  iamine-node models select\n  iamine-node models stats\n  iamine-node models download <model_id>\n  iamine-node models license accept <model_id> --yes\n  iamine-node models remove <model_id>\n  iamine-node semantic-eval\n  iamine-node regression-run\n  iamine-node check-code\n  iamine-node check-security\n  iamine-node validate-release\n  iamine-node tasks stats [--json]\n  iamine-node tasks trace <task_id> [--json]\n  iamine-node cluster status [--json]\n  iamine-node cluster stress [--requests N] [--concurrency N] [--task TYPE] [--required-model MODEL] [--batch-file PATH] [--prefix TEXT] [--timeout-secs N] [--output-dir PATH] [--profile standard|testnet-load-resilience] [--require-recovery-evidence] [--stop-on-first-failure] [--json]\n  iamine-node lan doctor [--json] [--network]\n  iamine-node lan infer <prompt> --model <model_id> [--max-tokens N]\n  iamine-node hardware inspect [--json] [--dynamic]\n  iamine-node hardware show [--json]\n  iamine-node hardware refresh [--yes] [--json] [--dynamic]\n  iamine-node --daemon [--bootnode ADDR] [--wan-peer ADDR] [--relay-policy MODE] [--relay-peer ADDR] [--testnet-admission MODE] [--testnet-allow-peer PEER_ID]\nFlags:\n  --bootnode ADDR or --bootnode=ADDR\n  --wan-peer ADDR or --wan-peer=ADDR\n  --relay-policy MODE or --relay-policy=MODE (disabled|operator-configured)\n  --relay-peer ADDR or --relay-peer=ADDR\n  --testnet-admission MODE or --testnet-admission=MODE (open|allowlist)\n  --testnet-allow-peer PEER_ID or --testnet-allow-peer=PEER_ID\n  --debug-network\n  --debug-scheduler\n  --debug-tasks\n  --force-network\n  --no-local\n  --prefer-local"
}
...
mod tests {
    ...
    fn cli_preserves_existing_help_text() {
        ...
        assert!(usage.contains("--wan-peer ADDR or --wan-peer=ADDR"));
        assert!(usage.contains("--relay-policy MODE or --relay-policy=MODE"));
        assert!(usage.contains("--relay-peer ADDR or --relay-peer=ADDR"));
        assert!(usage.contains("--testnet-admission MODE or --testnet-admission=MODE"));
        assert!(usage.contains("--testnet-allow-peer PEER_ID"));
        ...
        assert!(usage.contains("iamine-node support bundle [--output PATH] [--json]"));
        assert!(usage.contains("iamine-node agents node-doctor --package-root PATH [--json]"));
        ...
        assert!(usage.contains("[--profile standard|testnet-load-resilience]"));
